...
import com.ahmedmabrook.zloaderlibrary.request.utils.NetworkUtil;
import com.ahmedmabrook.zloaderlibrary.request.utils.StringUtils;
...
class ZrequestAsyncTask extends AsyncTask<Void, Void, Boolean> {

    private final String LOG_TAG = ZrequestAsyncTask.class.getSimpleName();
    ...
    ZrequestAsyncTask(Context context, String baseUrl, String endpoint, RequestType requestType,
    ...
    protected void onPreExecute() {
        super.onPreExecute();
        String cachedResponse = responseCache.getResponseFromCache(
                getCompletePath(buildPath(baseUrl, endpoint).build().toString()));
        if (StringUtils.stringIsNotEmpty(cachedResponse)) {
            if (onResponseListener != null) {
                onResponseListener.onSuccessResponse(cachedResponse, contentType, true);
            }
        }
    }
    ...
    protected void onPostExecute(Boolean result) {
        if (result) {
            responseCache.addResponseToCache(getCompletePath(
                    buildPath(baseUrl, endpoint).build().toString()), response);
            if (onResponseListener != null) {
                onResponseListener.onSuccessResponse(response, contentType, false);
            }
        } else {
            if (onResponseListener != null) {
                onResponseListener.onErrorResponse(error, message, code);
            }
        }
    }
    ...
    private String getCompletePath(String path) {
        ...
        try {
            decodedUrlInUTF = ZrequestAsyncTask.this.decodedUrlInUTF ?
